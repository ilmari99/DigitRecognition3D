...
def hyperopt_results_to_dataframe(folder):
    ...
    for log_dir in log_dirs:
        ...
        val_accuracy = trial_json['metrics']["metrics"]["val_accuracy"]["observations"][0]["value"][0]
        # Add the values to the dataframe
        if df is None:
            df = pd.DataFrame(columns = ["trial_id"] + hyperparams + ["val_loss", "val_accuracy"])
        df = pd.concat([df, pd.DataFrame([[trial_id] + values + [val_loss, val_accuracy]], columns = ["trial_id"] + hyperparams + ["val_loss", "val_accuracy"])])
    return df
...
if __name__ == "__main__":
    folder = "/home/ilmari/python/DigitRecognition3D/keras-tuner-logs_batch_size256_new2/digit_recognition/"
    ...
    plt.hist(df["val_accuracy"], bins = 20, label="val_accuracy")
    ...
    for col in df.columns:
        if col in ["trial_id", "val_loss", "val_accuracy", "tuner/bracket", "tuner/round","tuner/epochs", "tuner/trial_id"]:
            continue
